...
description: Agent skills for working with Minions Content-research MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-content-research data.
---

# Minions Content-research Agent Skills

Skills for agents operating on the `minions-content-research` toolbox.
...
```bash
# TypeScript
pnpm add @minions-content-research/sdk
...
The `content-research` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-content-research/sdk/schemas';
...
```python
from minions_content_research.schemas import custom_types
...
- This agent only writes to `minions-content-research` — it reads from other toolboxes but never writes to them
